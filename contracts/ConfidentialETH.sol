...
/// @notice Minimal mint-controlled confidential token for faucet distribution
contract ConfidentialETH is ConfidentialFungibleToken, SepoliaConfig {
    constructor() ConfidentialFungibleToken("Confidential ETH", "cETH", "") {}

    /// @notice Public mint without encryption to msg.sender
    function mint(uint64 amount) external {
        euint64 eamount = FHE.asEuint64(amount);
        _mint(msg.sender, eamount);
    }

    /// @notice Public mint with encrypted amount to msg.sender
